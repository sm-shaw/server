...
static size_t log_encrypt_buf(void *ctx,
  ...
  noexcept
{
  for (byte *l= buf; l != end; )
  {
    ...
    uint tlen= len;
    ut_a(MY_AES_OK == encryption_ctx_update(ctx, l, len, tmp, &tlen));
    // FIXME: this occasionally fails: ut_ad(!tlen);
    ut_ad(len == rlen);
    memcpy(l, tmp, rlen);
    // FIXME: tmp+= rlen;
    l+= rlen;
...
ATTRIBUTE_NOINLINE void mtr_t::encrypt()
{
  ...
  memset_aligned<8>(iv + 8, 0, (sizeof iv) - 8); // FIXME: include a page id
  ...
  m_log.for_each_block([&](mtr_buf_t::block_t *b)
  {
    ...
    {
    parse:
      // FIXME: this occasionally fails: ut_ad(t == tmp);
      size= log_encrypt_buf(ctx, t, buf, b->end());
      if (!size)
      {
        // FIXME: this occasionally fails: ut_ad(t == tmp);
        start_size= 0;
    ...
    else
    {
      ...
      //FIXME: t+= size;
      buf+= size;
      uint len= static_cast<uint>(t - tmp), tlen= len;
  ...
  // FIXME: this occasionally fails: ut_a(!len);
}
